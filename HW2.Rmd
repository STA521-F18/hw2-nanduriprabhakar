...
```{r setup, include=FALSE, options(warn=-1, message=-1)}
...
The data-set totally consists of 7 variables of which 6 of them viz, [ModernC, Change, PPgdp, Frate, Pop, Fertility] have missing values. All the 7 variables are quantitative variables. 

2. What is the mean and standard deviation of each quantitative predictor?  Provide in a nicely formatted table.

```{r}
# While calculating the mean and standard deviation of the quantitative predictor, we remove the NA values so that it does not affect measures of central tendency calculations.
...
```{r}
test = lm(ModernC ~ sqrt(Pop) + log(PPgdp) + Change + Frate + Fertility + Purban, data = UN3)
car::boxCox(test)
